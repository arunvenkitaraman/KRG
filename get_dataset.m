...
load('city45T.mat');
load('smhi18.mat');
smhi18_y=smhi18_y/max(abs(smhi18_y(:)));
smhi18_x=smhi18_x/max(abs(smhi18_x(:)));
T=[smhi18_y'];
alpvec=[0 logspace(-2,4,10)];
%alpvec=1;
...
city_ip=1:m;
city_op=1:m;
offset=0;  % The number of days offset between input and output temperatures
ns=60;
m=length(city_op);

set18=[1,2,3,10,12,4,14,7,8,9,11,16,17,19,20,21,23,25,27,31,33,35,36,44,45];
%% Constructing the adjacency matrix from the distance matrix A45
A=A45(set18,set18);
A=A.^2;
A=exp(-A/mean(A(:)));
A=A-diag(diag(A));
